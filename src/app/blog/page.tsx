...
export default function BlogIndex() {
  const publishedBlogs = blogs.filter((blog) => blog.published).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  ...
  return (
    <main className="min-h-screen pt-32 pb-48 px-4 overflow-hidden relative">
      ...
      <div className="max-w-4xl mx-auto flex flex-col items-center">
        ...
        <div className="flex flex-col gap-8 w-full animate-in fade-in slide-in-from-bottom-12 duration-1000 delay-150 fill-mode-both">
          {publishedBlogs.map((blog) => (
            ...
            >
              ...
              <div className="relative z-10">
                <p className="text-sm text-primary font-semibold mb-3">{new Date(blog.date).toLocaleDateString()}</p>
                <h2 className="text-2xl md:text-3xl font-bold mb-4 group-hover:text-primary transition-colors">{blog.title}</h2>
                <p className="text-foreground/70 leading-relaxed line-clamp-2 md:line-clamp-3">{blog.summary}</p>
